  ...
  audioInfo(QAudioDeviceInfo::defaultInputDevice())
{
  ...
  QCoreApplication::setOrganizationName("macaco");
  QSettings settings;
  settings.setValue("alo","maria");
  // prepares the playlist to be drawn
 ...
 // calculator->moveToThread(fftThread);
  ...
  // the current media id that is being played when it starts
  currentMediaId = -1;
  ...
  qDebug() << audioInfo.defaultOutputDevice().deviceName();
  qDebug() << audioInfo.defaultOutputDevice().supportedSampleSizes();
  fftThread->start();
  ...
  connect(this,SIGNAL(addFolderToLibrary(QString)),ui->library,SLOT(onAddFolder(QString)));
}
...
//  emit addFolderToLibrary(folder);
}
...
/*  static QTime time1, time2;
  time1 = time2;
  time2 = QTime().currentTime();
  elapsed = time1.msecsTo(time2);*/
//  qDebug() << "elapsed = " << ;
/*  if(elapsed < 100)
    elapsed = 100;*/
  // sample is the vector used to store the audio buffer
  // to be sent for fft calculation
  //sample.resize(buffer.sampleCount());
...
void MainWindow::metaDataChanged(){
  ...
  // perharps lots of metadata may be available, only the above are passed ahead
